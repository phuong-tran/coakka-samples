...
| Native file-transfer contract | The checksum-pinned `2.1.0+60ddf70d` package exercises a `9 * 1024 * 1024 + 731` byte multi-quantum loopback transfer through only the file-lane C ABI and checks SHA-256, durable receiver completion, progress, and counters. The exact public sample passes on macOS ARM64, Linux ARM64/x86-64 UTM hosts, and Windows x86-64 under Windows 11 ARM64 emulation. | `bash run.sh runtime-test file-lane` |
...
| Package-manager execution | npm Node.js and Bun `2.1.1` clean installs pass native request/reply; Node route-miss deadletter and Electron main-process intent also pass. NuGet `2.1.0` passes clean packaged request/reply/deadletter. PyPI `2.1.0` matches the staged SHA-256 and passes clean packaged request/reply. Go `v1.5.1` and Swift `v2.1.1` pass clean packaged request/reply and link the canonical public file-lane contract. The artifact mirror remains separately versioned. | [Runtime Package And Platform Evidence](runtime-package-platform-evidence.md) |
