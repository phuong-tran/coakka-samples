...
| Runtime tools | `coakka-client` and `coakka-runtime-inspect` `2.5.1+26f7944d` archives are published for all five native platforms and pass exact Runtime identity, architecture, dependency, archive, and checksum gates. Inspect has macOS ARM64 command plus `serve` evidence. The Client walkthrough and Inspect smoke pass from their published Docker Hub images on Linux amd64/arm64. Native matching-host execution is not claimed for the remaining archive targets. | `bash run.sh runtime-client`; `bash run.sh runtime-inspect published-smoke`; [Runtime tools 2.5.1](releases/2026-08-21-runtime-tools-2.5.1.md) |
| Package-manager execution | Maven Central Runtime, Spring Boot, Quarkus, npm Node/Bun/Electron, PyPI, and NuGet Runtime `2.5.3` are public over connector source `0ba485e` and Core `2.5.1+26f7944d`; Go `v1.8.3`, SwiftPM `v2.5.3`, and the static connector archive train use the same Core. Registry-downloaded npm tarballs and the PyPI wheel are byte-identical to their candidates. NuGet.org repository-signs the candidate, adds only `.signature.p7s`, and all other ZIP entries remain byte-identical. Clean Node, Bun, Python, and .NET consumers execute RuntimeInfo and request/reply; Electron resolves the exact Node dependency and package surface. Android `1.2.0` remains a signed internal, release-minified candidate only; no Maven publication is planned. Logger coordinates remain independently versioned. | [Runtime registry packages 2.5.3](releases/2026-08-21-runtime-registries-2.5.3.md), [Runtime Package And Platform Evidence](runtime-package-platform-evidence.md), and [JVM Logger Maven Central 1.2.2](releases/2026-08-17-logger-jvm-maven-central-1.2.2.md) |
