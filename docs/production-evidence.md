...
| Package-manager execution | npm Node.js and Bun `2.3.0` clean installs pass native request/reply; Node route-miss deadletter and Electron main-process intent also pass. npm, Go `v1.6.0`, and Swift `v2.3.0` carry the exact five-platform `2.3.0+a83ab412` payload and expose File Lane and Stream Lane. NuGet `2.1.0` passes repository-signature verification and clean packaged request/reply/deadletter. PyPI `2.1.0` matches its staged SHA-256 and passes clean packaged request/reply. | [Runtime Package And Platform Evidence](runtime-package-platform-evidence.md) |
