...
| Native file-transfer contract | Public C11 source exercises a `9 * 1024 * 1024 + 731` byte multi-quantum loopback transfer through only the file-lane C ABI and checks SHA-256, durable receiver completion, progress, and counters. The exact promoted archive `2.1.0+60ddf70d` passes on macOS ARM64, Linux ARM64/x86-64, and Windows x86-64 under Windows 11 ARM64 emulation. | `bash run.sh runtime-test file-lane` |
...
| Package-manager execution | npm Node.js and Bun `2.1.1` clean installs pass native request/reply; Node route-miss deadletter and Electron main-process intent also pass. NuGet `2.1.0` passes repository-signature verification and clean packaged request/reply/deadletter. PyPI `2.1.0` matches the staged SHA-256 and passes clean packaged request/reply. Go `v1.5.1` passes packaged request/reply on macOS ARM64 and Linux ARM64/x86-64; Swift `v2.1.1` passes its macOS ARM64 package smokes. Both documentation-integrity patches use the canonical public file-lane contract. | [Runtime Package And Platform Evidence](runtime-package-platform-evidence.md) |
