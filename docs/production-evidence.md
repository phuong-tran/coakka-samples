...
| SFTP artifact publisher addon | Native addon `0.2.0+c5656cc8` loads beside Runtime `2.3.0` on Linux ARM64/x86-64, macOS ARM64, and Windows 11 ARM64/x86-64. Every module exposes exactly 11 reviewed C symbols, has no ambient libssh2/libcrypto/libssl dependency, and passes matching-host pinned SFTP acquisition, integrity/no-clobber, failure, cancellation, and File Lane delivery evidence. | `bash runtime-addons/artifact-publisher-sftp/native/run.sh published` |
