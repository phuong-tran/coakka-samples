...
| Package-manager execution | npm Node.js and Bun `2.1.1` clean installs pass native request/reply; Node route-miss deadletter and Electron main-process intent also pass. NuGet `2.1.0` passes clean packaged request/reply/deadletter; PyPI `1.4.6` retains its recorded evidence. Go `v1.5.0`, Swift `v2.1.0`, and the artifact mirror remain separately versioned. | [Runtime Package And Platform Evidence](runtime-package-platform-evidence.md) |
