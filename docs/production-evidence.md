...
| Native file-transfer contract | Public C11 source exercises a `9 * 1024 * 1024 + 731` byte multi-quantum loopback transfer through only the File Lane C ABI and checks SHA-256, durable receiver completion, progress, and counters. The exact promoted archive `2.3.0+a83ab412` passes the five-platform release gates and the public macOS consumer. | `bash run.sh runtime-test file-lane` |
| SFTP artifact publisher addon | Native addon `0.1.0+40810b79` for macOS ARM64 loads beside Runtime `2.3.0`, exposes exactly 11 reviewed C symbols, has no ambient libssh2/libcrypto/libssl dependency, and passes pinned-host SFTP acquisition, integrity/no-clobber checks, failure/cancellation paths, plus cross-process File Lane delivery from Service A to Service B. Linux and Windows are not claimed by this release. | `bash runtime-addons/artifact-publisher-sftp/native/run.sh published` |
...
| Package-manager execution | npm Node.js and Bun `2.3.0` clean installs pass native request/reply; Node route-miss deadletter and Electron main-process intent also pass. NuGet `2.3.0` passes repository-signature verification, exact candidate-entry comparison, and clean request/reply/deadletter. PyPI `2.3.0` is byte-identical to its staged wheel and passes clean request/reply, deadletter, hot reload, and local desktop CRUD. npm, PyPI, NuGet, Go `v1.6.0`, and Swift `v2.3.0` carry the exact five-platform `2.3.0+a83ab412` payload and expose File Lane and Stream Lane. | [Runtime Package And Platform Evidence](runtime-package-platform-evidence.md) |
