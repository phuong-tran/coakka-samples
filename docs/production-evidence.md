...
| Native file-transfer contract | Public C11 source exercises a `9 * 1024 * 1024 + 731` byte multi-quantum loopback transfer through only the file-lane C ABI and checks SHA-256, durable receiver completion, progress, and counters. This is source-level `2.1.0` preparation until the compatibility matrix lists exact promoted artifacts. | `COAKKA_NATIVE_EVIDENCE_RUNTIME_SOURCE_DIR=../coakkaCoreNativeDev/v2 bash run.sh runtime-test file-lane` |
