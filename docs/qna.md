...
## Does CoAkka Replace Nginx, Ingress, Or Service Load Balancing?

No.

Edge load balancers and CoAkka route strategies operate at different layers.

Short answer:

```text
Nginx, Kubernetes Ingress, or a cloud load balancer distributes external HTTP
traffic across app instances.

CoAkka routes internal capability delivery after a request has entered an app
process.
```
...
```text
external client
  -> Nginx / Ingress / Service LB
  -> app instance
  -> CoAkka runtime
  -> target route strategy
  -> local handler or peer runtime
```

That means CoAkka can benefit from edge load balancing when external traffic is
already spread across app instances. It still answers a different question once
the request is inside an app process:

- which `target` should receive this internal work?
- which route generation is active?
- is the selected endpoint `LOCAL`, remote, or `UNAVAILABLE`?
- should selection use `SINGLE_OWNER`, `WEIGHTED_ROUND_ROBIN`, or
  `RENDEZVOUS_HASH`?
- if delivery fails, what deadletter reason should be exposed?

Do not say:

```text
CoAkka replaces Nginx.
```

Say:

```text
Nginx or Ingress balances external HTTP ingress. CoAkka routes internal runtime
capabilities by target, route snapshot, endpoint flags, and route strategy.
```
