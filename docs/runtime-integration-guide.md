...
The current public runtime artifact reports `backend=stub`. Local in-process
request/reply samples work. Remote cross-process delivery is represented in the
customer scenarios, but the web services use an HTTP store fallback after the
runtime route returns a delivery deadletter.

Treat that fallback as demo glue, not the production architecture. A production
runtime integration should use the runtime route path when a remote-capable
backend is published, and should disable or remove the HTTP fallback.
...
| `runtime-deadletter-http-fallback` | The runtime route produced a deadletter and the sample used the HTTP fallback. |
