...
ARM64, Linux x86_64, and Linux ARM64, and `1.3.1+6c63864` for Windows x86_64.
The sample runner verifies those archives through the same manifest/checksum
resolver used by the runtime-client lane. Windows ARM64 inspect archives should
be added only after platform-native package verification exists for that host.
