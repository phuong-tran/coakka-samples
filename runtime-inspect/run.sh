...
COAKKA_RUNTIME_INSPECT_RELEASE_ID="${COAKKA_RUNTIME_INSPECT_RELEASE_ID:-2.5.1+26f7944de4a4e0598845a54e4775f9463a9e33be}"
COAKKA_RUNTIME_INSPECT_RELEASE_VERSION="${COAKKA_RUNTIME_INSPECT_RELEASE_VERSION:-2.5.1}"
COAKKA_RUNTIME_INSPECT_DOCKER_IMAGE_DEFAULT="coakka-runtime-inspect-sample:${COAKKA_RUNTIME_INSPECT_RELEASE_VERSION}-local"
COAKKA_RUNTIME_INSPECT_DOCKERHUB_IMAGE_DEFAULT="docker.io/gabrielgun1983/coakka-runtime-inspect-sample:2.5.1-26f7944d-remote"
...
require_self_contained_dockerfiles() {
  ...
  if [[ -n "${matches}" ]]; then
    printf '%s\n' "${matches}" >&2
    coakka_die "runtime-inspect Docker images must use self-contained native archives and must not install native runtime packages."
  fi
}
