...
def ask(orchestrator: ConnectorOrchestrator, target: str, message: str) -> dict:
    return orchestrator.ask_json(
...
def expect_route_miss(orchestrator: ConnectorOrchestrator, target: str) -> dict:
    try:
        ask(orchestrator, target, "route-miss")
...
def main() -> None:
    ...
    with ConnectorOrchestrator.start(start_spec=start_spec) as orchestrator:
        ...
        orchestrator.register_handler(V1_TARGET, v1_handler)
        orchestrator.register_handler(V2_TARGET, v2_handler)

        initial = ask(orchestrator, V1_TARGET, "before-reload")
        print(f"coakka_runtime_hot_reload_before payload={json.dumps(initial, separators=(',', ':'))}")

        initial_miss = expect_route_miss(orchestrator, V2_TARGET)
        print(
            "coakka_runtime_hot_reload_initial_miss "
            f"target={initial_miss['target']} generation={initial_miss['generation']}"
        )

        before_stale = orchestrator.stats()
        expect_control_rejected(
            "stale",
            lambda: orchestrator.control.apply_snapshot(
                generation=1,
        ...
        after_stale = orchestrator.stats()
        ...
        orchestrator.control.apply_snapshot(
            generation=2,
            routes=[local_route(V2_TARGET, 19511)],
            source_connector="python-hot-reload-sample",
            seq=3,
        )
        monitor = orchestrator.monitor.await_applied_generation_at_least(2, timeout_ms=1000)
        if monitor is None:
            raise RuntimeError("generation 2 was not observed by monitor")
        after_reload = orchestrator.stats()
        ...
        after_invalid = orchestrator.stats()
        ...
        final_miss = expect_route_miss(orchestrator, MISSING_TARGET)
