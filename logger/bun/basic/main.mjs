...
const info = BunLogger.readInfo();
console.log(`coakka_logger_info abi=${info.abiVersion} version=${info.runtimeVersion} git=${info.gitCommit}`);

const logger = BunLogger.start({ systemName: "bun-sample-logger", minLevel: CoakkaLoggerLevel.INFO });
try {
  const category = "samples.logger.bun.basic";
  const message = '{"event":"hello","language":"bun"}';
