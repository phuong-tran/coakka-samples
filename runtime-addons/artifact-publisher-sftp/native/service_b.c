...
static int wait_for_receive(coakka_v2_file_lane_t *lane,
  ...
  for (attempt = 0; attempt < SAMPLE_WAIT_ATTEMPTS; ++attempt) {
    ...
    /* Wait on the retained record's update sequence instead of polling the
       transfer state with a sleep loop. */
...
int main(void) {
  ...
  if (bind_host == NULL || port_text == NULL || transfer_id == NULL ||
      ...
      sample_parse_sha256(digest_text, digest) != 0 ||
...

  {
    ...
    config.max_file_size = expected_size + 4096;
  ...
  /* Readiness is published only after the exact one-use receive grant exists;
     Service A can never race ahead of receiver authorization. */
  if (sample_write_ready_file(ready_file, bound_port) != 0) {
    goto cleanup;
  }
  printf("coakka_sftp_service_b_ready transfer=%s host=%s port=%u\n",
  ...
  if (snapshot.state != COAKKA_V2_FILE_TRANSFER_STATE_COMPLETED ||
    ...
    fprintf(
        stderr,
        "receiver terminal failure: state=%u result=%u bytes=%llu detail=%s\n",
  ...
  printf("coakka_sftp_service_b_completed transfer=%s bytes=%llu state=%u "
         "result=%u\n",
         transfer_id, (unsigned long long)received_size, snapshot.state,
         snapshot.result);
