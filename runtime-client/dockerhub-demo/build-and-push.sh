...
image="${COAKKA_RUNTIME_CLIENT_DEMO_IMAGE:-docker.io/gabrielgun1983/coakka-runtime-client-demo:${immutable_tag}}"
[[ "${release_version}" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || \
  coakka_die "Docker Hub client context has an invalid release version."
[[ "${samples_source_commit}" =~ ^[0-9a-f]{40}$ ]] || \
  coakka_die "Docker Hub client context has an invalid Samples source commit."
[[ "${samples_source_dirty}" == "false" ]] || \
  coakka_die "Docker Hub client publication requires a clean Samples source commit."
[[ "${runtime_generation}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\+[0-9a-f]{40}$ ]] || \
  coakka_die "Docker Hub client context has an invalid Runtime generation."
[[ "${runtime_generation%%+*}" == "${release_version}" ]] || \
  coakka_die "Docker Hub client context version and Runtime generation differ."
[[ "${image##*:}" == "${immutable_tag}" && "${image}" != *@* ]] || \
  coakka_die "Docker Hub client image must use immutable tag ${immutable_tag}."
...
grep -Eq 'Platform:[[:space:]]+linux/arm64' <<<"${manifest_report}" || \
  coakka_die "Docker Hub client image has no linux/arm64 manifest."
for platform in linux/amd64 linux/arm64; do
  docker run --rm --platform "${platform}" \
    ...
    "grep -Fx 'samples_source_commit=${samples_source_commit}' /opt/coakka-image/RELEASE.txt >/dev/null \
      ...
      && test -f /opt/coakka/LICENSE \
      && test -f /opt/coakka/NATIVE-LICENSE.md \
      && test -f /opt/coakka/PACKAGE-LICENSE.md \
      && test -f /opt/coakka/NOTICE \
      && test -f /opt/coakka-demo/customer-service/LICENSE \
      && test -f /opt/coakka-demo/customer-service/NATIVE-LICENSE.md \
      && test -f /opt/coakka-demo/customer-service/PACKAGE-LICENSE.md \
      && test -f /opt/coakka-demo/customer-service/NOTICE"
  docker run --rm --platform "${platform}" \
    "${image}@${image_digest}" walkthrough
done
...
live_gate=walkthrough
EOF
echo "[coakka-runtime-client-demo] pushed ${image}@${image_digest}"
