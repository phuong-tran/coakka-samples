...
Usage:
  ...
  bash run.sh runtime-test [smoke|pressure|stress|soak|connection-strategies|file-lane-simple|file-lane-owner-aware|stream-lane-simple|stream-lane-owner-aware|race|hot-reload]
...
case "$1" in
  ...
  logger)
    if [[ "$#" -eq 1 || "${2:-}" == "all" ]]; then
      ...
      coakka_die "Usage: bash run.sh logger [<jvm|python|node|bun|electron|tauri|go|csharp|rust|zig|mojo|native>] <sample>"
    fi
    ;;
  runtime)
    if [[ "$#" -eq 1 || "${2:-}" == "all" ]]; then
