...
backend = runtime["runtimeInfo"]["southboundBackend"]
demo_mode = runtime["connector"]["demoMode"]
route_count = runtime["runtimeConfig"]["routeCount"]
if backend != "caf":
    raise SystemExit(f"[fail] expected runtime backend caf, got {backend!r}")
...
print("[ok] Spring Boot starter local backend is caf with 4 local capability routes")
PY
